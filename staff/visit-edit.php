<?php
/**
 * SAMRIDHI AGRO - Staff Edit Visit
 * 
 * This page allows staff members to update existing visit details.
 * 
...
// If no ID or invalid ID, redirect to visits list
if ($visitId <= 0) {
...
// Check if visit can be edited (only planned or in_progress)
if (!in_array($visit['status'], ['planned', 'in_progress'])) {
    setFlashMessage('error', 'This visit cannot be edited as it is already completed or cancelled.');
    redirect('staff/visits.php');
    exit;
}
...
$formData = [
    ...
    'shop_id' => $visit['shop_id'] ?? 0,
    'visit_date' => $visit['visit_date'],
    'visit_time' => $visit['visit_time'] ? date('H:i', strtotime($visit['visit_time'])) : '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST[CSRF_TOKEN_NAME]) || !verifyCsrfToken($_POST[CSRF_TOKEN_NAME])) {
        ...
        redirect('staff/visit-edit.php?id=' . $visitId);
        exit;
    }
    ...
    // If no errors, update visit
    if (!$hasErrors) {
        try {
            $sql = "UPDATE staff_visits SET 
                        agent_id = ?,
                        shop_id = ?,
                        visit_type = ?,
                        visit_date = ?,
            ...
            logActivity(
                ...
                $_SESSION['user_id'],
                'visits',
                'Updated visit #' . $visitId . ' - ' . $formData['visit_type'] . ' on ' . $formData['visit_date']
            );
            ...
            setFlashMessage('error', 'Failed to update visit. Please try again.');
            redirect('staff/visit-edit.php?id=' . $visitId);
            exit;
        }
    }
...
<style>
    ...
    .visit-info-box .info-item .label {
        color: #6B7A7B;
    }
    
    .visit-info-box .info-item .value {
        color: #052E16;
        font-weight: 500;
    }
...
<div class="content-card">
    <div class="card-header">
        ...
        <div style="display: flex; gap: 8px;">
            <a href="visit-view.php?id=<?php echo $visit['id']; ?>" class="card-action">
    ...
    <div class="visit-info-box">
        <div class="info-item">
            ...
            <span class="value"><?php echo formatDate($visit['created_at']); ?></span>
        </div>
        <div class="info-item">
    ...
    <form method="POST" action="" id="visitForm" novalidate>
        ...
        <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid #E5EDE7; display: flex; gap: 12px;">
            <button type="submit" class="btn-primary" id="submitBtn">
                <i class="fas fa-save"></i> <span id="btnText">Update Visit</span>
