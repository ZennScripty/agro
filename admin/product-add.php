...
// ============================================
requirePermissionOrAdmin('product.create', 'product-add.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($formData['product_slug'])) {
        ...
        $sql = "SELECT id FROM products WHERE product_slug = ?";
        $existing = $db->fetchOne($sql, [$formData['product_slug']]);
        if ($existing) {
    ...
    if (empty($formData['sku'])) {
        ...
        $existing = $db->fetchOne($sql, [$formData['sku']]);
        if ($existing) {
    ...
    if (!$hasErrors) {
        try {
            ...
            redirect('admin/product-add.php');
            exit;
        }
    }
}
    ...
    <form method="POST" action="" id="productForm" enctype="multipart/form-data" novalidate>
        ...
        <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid #E5EDE7; display: flex; gap: 12px;">
            <button type="submit" class="btn-primary" id="submitBtn">
                <i class="fas fa-save"></i> <span id="btnText">Create Product</span>
