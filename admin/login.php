    ...
    if (!isset($_POST[CSRF_TOKEN_NAME]) || !verifyCsrfToken($_POST[CSRF_TOKEN_NAME])) {
        ...
        if (empty($username) || empty($password)) {
            ...
            if ($authResult['success']) {
                ...
                if (hasRole('admin')) {
                    error_log('User has admin role. Redirecting to dashboard.');
                    ...
                    error_log('User does NOT have admin role. Role: ' . $_SESSION['user_role']);
                    // Logout non-admin user
                    logoutUser();
                    $error = 'You do not have admin access. Please use the correct portal.';
                }
            } else {
                $error = $authResult['error'] ?? 'Invalid username or password.';
                error_log('Authentication failed: ' . $error);
            }
        }
    }
}
...
<head>
    ...
    <title>Admin Login - Samridhi Agro</title>
